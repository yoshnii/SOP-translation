...
import translator_core as core   # 复用保格式管线
...
# token 静默多久判定为"卡死"(秒)。只要这段时间内有任意 token 到达就重置。
# 设大一点(90s)很安全:正常慢请求每隔几秒就有 token,不会触发;真断流才触发。
STALL_SECONDS = 90


def _stream_with_stall_guard(client, model, system, user):
    """流式调用 + token 静默检测。返回完整译文文本。

    用流式(stream=True)逐 token 接收。维护"最后一次收到 token 的时刻",
    若超过 STALL_SECONDS 没有新 token,抛 TimeoutError(判定真卡死,由上层重试)。
    正常的慢请求(token 持续到达)不会触发 —— 这就是"区分慢和死"的关键。
    """
    import time
    resp = client.chat.completions.create(
        model=model, temperature=0.2, max_tokens=8000, stream=True,
        # 整体请求也设个大兜底超时,防极端情况(远大于单 token 间隔)
        timeout=600,
    ...
    for chunk in resp:
        ...
        if delta:
            parts.append(delta)
            last = time.time()          # 收到 token,重置静默计时
        elif time.time() - last > STALL_SECONDS:
            raise TimeoutError(f"token 静默超过 {STALL_SECONDS}s,判定卡死")
...
def translate_chunk(client, model, segments, idxs, glossary=None):
    ...
    # 流式输出 + "token 静默"卡死检测:
    #   模型边翻边吐 token。只要有 token 在流(哪怕很慢)就是活的,不重试;
    #   只有 STALL_SECONDS 内一个 token 都没来,才判定真卡死、重试。
    #   —— 这样能区分"V4-Pro 慢慢翻"(token 持续来)和"真卡死"(token 断流),
    #      不会误杀正在工作的慢请求。
    out = None
    last_err = None
    for attempt in range(3):
        try:
            out = _stream_with_stall_guard(client, model, system, marked)
            break
        except Exception as e:
            last_err = e
            print(f"    块请求第{attempt+1}次失败({type(e).__name__}),重试...")
    if out is None:
        # 3次都失败:保留原文,不让整个任务卡死
        print(f"    块翻译彻底失败,保留原文: {last_err}")
        return {idxs[k]: segments[idxs[k]].text for k in range(len(idxs))}
    # 按 [Pn] 拆回
...
def translate_doc(in_path, out_path, model, progress_cb=None,
                  glossary_path="glossary.csv", return_stats=False):
    ...
    # 扫描译文里还含中日韩字符的,并行补翻(并更新进度,避免用户以为卡死)。
    residual = []
    for i, seg in enumerate(segments):
        ...
        if re.search(r"[一-鿿぀-ヿ가-힯]", en):
            residual.append(i)
    # 数量上限:残留过多说明可能有系统性问题,补翻全部会非常慢,只补前 N 段。
    MAX_BACKFILL = 60
    if len(residual) > MAX_BACKFILL:
        print(f"  残留中文 {len(residual)} 段过多,只补翻前 {MAX_BACKFILL} 段")
        residual = residual[:MAX_BACKFILL]
    if residual:
        print(f"  残留中文 {len(residual)} 段,并行补翻...")
        total_bf = len(residual)
        done_bf = [0]
        def backfill(i):
            try:
                ...
                pass
            done_bf[0] += 1
            # 补翻阶段进度:显示成"补翻 x/y"(用 total+done 编码,前端会显示>100%或单独提示)
            if progress_cb:
                progress_cb(len(chunks) + done_bf[0], len(chunks) + total_bf)
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            list(ex.map(backfill, residual))
    ...
    if return_stats:
        # 字符统计(用于费用)
        uniq_zh = list(dict.fromkeys(s.text for i, s in enumerate(segments) if not core.should_skip(s.text)))
        in_chars = sum(len(re.sub(r"\s+", " ", t).strip()) for t in uniq_zh)
        out_chars = sum(len(post_process(cache.get(i, "")))
                        for i, s in enumerate(segments) if not core.should_skip(s.text))
        return {"out_path": out_path, "in_chars": in_chars, "out_chars": out_chars,
                "residual": n_left, "chunks": len(chunks)}
    return out_path
